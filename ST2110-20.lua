...
do  
    ...
    F.Length = ProtoField.uint16("st_2110_20.Length","Length",base.DEC,nil)
    F.FID = ProtoField.bool("st_2110_20.FID","Field Identification",8,{"Second field","First field"},0x80)
    F.LineNo = ProtoField.uint16("st_2110_20.LineNo","Line Number",base.DEC,nil,0x7FFF)
    F.Cont = ProtoField.bool("st_2110_20.Cont","Continuation",8,{"Yes","No"},0x80)
    F.Offset = ProtoField.uint16("st_2110_20.Offset","Offset",base.DEC,nil,0x7FFF)
	...
	if totalLength % 180 == 0 then 
		subtree:append_text(":Sample Row Data Segments length sum n*180 - BPM")
	else
		subtree:append_text(":Sample Row Data Segments length sum not n*180 - GPM")
	end
        Offset=0
